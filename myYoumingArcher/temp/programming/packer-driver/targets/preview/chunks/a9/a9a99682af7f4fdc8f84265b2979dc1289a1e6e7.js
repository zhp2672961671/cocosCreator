  ...
  return {
    ...
    execute: function () {
      ...
      CSV = {
        ...
        // 将行数组转化为对象 field行属性数组，sample行案例数组cast行值类型数组
        ...
        parse: function parse(text, opts, fn) {
          ...
          if (!opts.delimiter || !opts.newline) {
            var limit = Math.min(48, Math.floor(text.length / 20), text.length); // 分割符

            opts.delimiter = opts.delimiter || this.mostFrequent(text, CELL_DELIMITERS, limit); // 换行符
...
//# sourceMappingURL=a9a99682af7f4fdc8f84265b2979dc1289a1e6e7.js.map